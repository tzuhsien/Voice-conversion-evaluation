...
def get_spectrograms(fpath, multi=False):
    ...
    # Loading sound file
    if multi == True:
        y, sr = librosa.load(fpath[0], sr=hp.sr)
        for f in fpath[1:]:
            tmp, sr = librosa.load(f, sr=hp.sr)
...
def wav2melspectrogram(wav):
    ...
    # stft
    linear = librosa.stft(
        y=wav, n_fft=hp.n_fft, hop_length=hp.hop_length, win_length=hp.win_length
    )
