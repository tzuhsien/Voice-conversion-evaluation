...
class AudioProcessor:
    ...
    def file2spectrogram(cls, file_path, return_wav=False):
        ...
        trim_wav = trim(wav, top_db=cls.top_db)[0]
        trim_wav = filtfilt(*cls.butter_highpass(), trim_wav)
        trim_wav = trim_wav * 0.96

        d_mag = cls.short_time_fourier_transform(trim_wav)
        ...
        if return_wav:
            return wav, np.clip(db_normalized, 0, 1).astype(np.float32)
        else:
