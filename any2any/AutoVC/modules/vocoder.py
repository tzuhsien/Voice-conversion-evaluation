...
class Vocoder(nn.Module):
    def __init__(self, mel_channels=80, conditioning_channels=128, embedding_dim=256, rnn_channels=896, fc_channels=512, bits=9, hop_length=256):
        super().__init__()
        self.rnn_channels = rnn_channels
        self.quantization_channels = 2 ** bits
        self.hop_length = hop_length

        self.rnn1 = nn.GRU(mel_channels, conditioning_channels,
                           num_layers=2, batch_first=True, bidirectional=True)
        self.embedding = nn.Embedding(
            self.quantization_channels, embedding_dim)
        self.rnn2 = nn.GRU(
    ...
    def forward(self, x, mels):
        ...
        mels = F.interpolate(mels.transpose(
            1, 2), scale_factor=self.hop_length)
    ...
    def generate(self, mel):
        ...
        with torch.no_grad():
            ...
            mel = F.interpolate(mel.transpose(
                1, 2), scale_factor=self.hop_length)
            ...
            x = torch.zeros(batch_size, device=mel.device).fill_(
                self.quantization_channels // 2).long()

            for m in tqdm(torch.unbind(mel, dim=1)):
                x = self.embedding(x)
                ...
                x = dist.sample()
                output.append(2 * x.float().item() /
                              (self.quantization_channels - 1.) - 1.)
