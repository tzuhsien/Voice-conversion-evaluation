...
def softclamp(x, mx=1, margin=0.03, alpha=0.7, clipval=100):
    ...
    x = mask*x+(1-mask)*torch.sign(x) * \
        ((1-torch.exp(-alpha*(xabs-rmargin)/margin))*margin+rmargin)
    return x
...
class CouplingNet(torch.nn.Module):
    ...
    def forward(self, h, emb):
        ...
        h = torch.nn.functional.conv1d(h.view(
            1, -1, lchunk), w, bias=b, padding=self.kw//2, groups=sbatch*nsq).view(sbatch, self.ncha, lchunk)
        # """
